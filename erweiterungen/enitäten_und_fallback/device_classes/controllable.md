...
|---|---|---:|---|---|
| `input_number.ems_<prefix>_min_technisch_<u>` | W oder A | ja | Muss als Fallback-Feld in der App/Addon Config vorhanden sein  | Untere technische Grenze; Werte darunter rasten auf `0` oder auf das Minimum |
| `input_number.ems_<prefix>_max_technisch_<u>` | W oder A | ja | Muss als Fallback-Feld in der App/Addon Config vorhanden sein  | Obere technische Grenze; mit `0` kann das Gerät keine Leistung erhalten |
...
| `input_number.ems_<prefix>_hoch_regelzeit_s` | s | ja | Muss als Fallback-Feld in der App/Addon Config vorhanden sein  | Mindestabstand beim Erhöhen des Sollwerts |
| `input_number.ems_<prefix>_runter_regelzeit_s` | s | ja | Muss als Fallback-Feld in der App/Addon Config vorhanden sein  | Mindestabstand beim normalen Absenken; bei Defizit wird sofort abgesenkt |
| `input_number.ems_<prefix>_max_anderung_pro_schritt_<u>` | W oder A | ja | Muss als Fallback-Feld in der App/Addon Config vorhanden sein  | Maximale Sollwertänderung je Regelzyklus |
| `input_number.ems_<prefix>_min_anderung_pro_schritt_<u>` | W oder A | ja | Muss als Fallback-Feld in der App/Addon Config vorhanden sein  | Totband; kleinere Änderungen werden nicht geschrieben |
